...
class DecisionTransformer(pl.LightningModule):
    def __init__(self, d_model, n_head, n_layers, clip_model, vqgan_model, output_resolution):
        super().__init__()
        ...
        self.clip_embedding_linear = torch.nn.Linear(clip_model.visual.output_dim, d_model)
    ...
    def training_step(self, batch, batch_idx):
        ...
        inputs = torch.cat([targets_e.unsqueeze(1), cos_sims_e.unsqueeze(1), tokenses_e], axis = 1)
        ...
        vqgan_probs = self.decoder_vqgan_tokens(encoded)[:, 2:] #logged and unnormalized probabilities
        ...
        patches_loss = F.cross_entropy(vqgan_probs.reshape(-1, self.vqgan_model.quantize.n_e), tokenses.reshape(-1))
    ...
    def setup_positional_encoding(self):
        ...
        self.register_buffer('positional', torch.zeros(self.d_model).repeat(self.vqgan_tokens + 2, 1))
...
def test_dummy_train():
    ...
    dummy_vqgan_tokens = torch.zeros(16, dtype=torch.long).unsqueeze(0).repeat(copies, 1)
